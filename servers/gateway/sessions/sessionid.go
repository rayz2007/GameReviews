...
func NewSessionID(signingKey string) (SessionID, error) {
	...
	if len(signingKey) == 0 {
		return InvalidSessionID, errors.New("the signingKey cannot be empty")
	}
	...
	if err != nil {
		fmt.Errorf("error generating random bytes: %v\n", err)
	}
	hash := hmac.New(sha256.New,[]byte(signingKey))
	hash.Write(randomBytes)
	signature := hash.Sum(nil)
	sessionByteSlice := append(randomBytes, signature...)
	resID := base64.URLEncoding.EncodeToString(sessionByteSlice)
	return SessionID(resID), nil
}
...
func ValidateID(id string, signingKey string) (SessionID, error) {
	...
	if err != nil {
		fmt.Errorf("error generating decoding id parameter: %v\n", err)
	}
	if len(decoded) != 64 {
		return InvalidSessionID, ErrInvalidID
	}
	idPortion := decoded[:idLength]
	hashPortion := decoded[idLength:]
	hashId := hmac.New(sha256.New,[]byte(signingKey))
	hashId.Write(idPortion)
	signature := hashId.Sum(nil)
	if hmac.Equal(signature, hashPortion) {
